...
void compilecons(void)
{
   ...
   /* The stock GAME.CON/USER.CON/DEFS.CON live inside duke3d.grp. Keep
    * custom /xFILE overrides on the filesystem, but default to the GRP set. */
   if (stricmp(confilename, "GAME.CON") == 0)
       readfromGRP = 1;

   loadefs(userconfilename,mymembuf, readfromGRP);  
...
void findGRPToUse(char * groupfilefullpath){
    ...
    if (getGameDir()[0] != '\0')
    {
        char defaultGroupFile[512];

        snprintf(defaultGroupFile, sizeof(defaultGroupFile), "%s/%s", getGameDir(), "duke3d.grp");
        if (access(defaultGroupFile, F_OK) == 0)
        {
            snprintf(groupfilefullpath, 512, "%s", defaultGroupFile);
            return;
        }
    ...
    while ((dirEntry = readdir(dir)) != NULL)
    {
        ...
        if (dukeGRP_Match(dirEntry->d_name, (int32_t)strlen(dirEntry->d_name)))
        {
