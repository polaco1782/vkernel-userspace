...
static char* get_effective_game_dir(void)
{
#ifdef DUKE_DEFAULT_GAME_DIR
    if (game_dir[0] == '\0')
    {
        strncpy(game_dir, DUKE_DEFAULT_GAME_DIR, sizeof(game_dir) - 1);
...
static int grp_filename_matches(const uint8_t* entry_name, const char* filename)
{
    int i;

    for (i = 0; i < 12; ++i)
    {
        const unsigned char entry_ch = entry_name[i];
        const unsigned char file_ch = (unsigned char)filename[i];

        if (file_ch == '\0')
            return entry_ch == '\0';

        if (toupper(entry_ch) != toupper(file_ch))
            return 0;
...
int32_t kopen4load(const char  *filename, int openOnlyFromGRP){
	...
	for(k=grpSet.num-1;k>=0;k--)
	{
        ...
        for(i=archive->numFiles-1;i>=0;i--){
            ...
            if (grp_filename_matches(archive->gfilelist[i], filename)){
...
int32_t TCkopen4load(const char  *filename, int readfromGRP)
{
	...
	int32_t result = 0;
    char* current_game_dir = get_effective_game_dir();
    
	if(current_game_dir[0] != '\0' && !readfromGRP)
	{
		sprintf(fullfilename, "%s\\%s", current_game_dir, filename);
...
void   setGameDir(char* gameDir){
    ...
    strncpy(game_dir,gameDir,sizeof(game_dir) - 1);
    game_dir[sizeof(game_dir) - 1] = '\0';
}

char*  getGameDir(void){
    return get_effective_game_dir();
}
