...
public:
    ...
    inline ImVec4 getColor()
    {
        ...
        case ImGuiToastType::Error:
            return {1.0f, 0.0f, 0.0f, 1.0f}; // Error
        case ImGuiToastType::Info:
            return {0.0f, 157.0f / 255.0f, 1.0f, 1.0f}; // Blue
        default:
            return {1.0f, 1.0f, 1.0f, 1.0f}; // White
        }
    }
    ...
    inline ImGuiToastPhase getPhase()
    {
        ...
        {
            return elapsed > NOTIFY_FADE_IN_OUT_TIME ? ImGuiToastPhase::Wait : ImGuiToastPhase::FadeIn;
        }
...
public:
    ...
    ImGuiToast(ImGuiToastType type, int dismissTime = NOTIFY_DEFAULT_DISMISS)
    {
        ...
        memset(this->buttonLabel, 0, sizeof(this->buttonLabel));
    }
    ...
    inline void RenderNotifications()
    {
        ...
        for (size_t i = 0; i < notifications.size(); )
        {
            ...
            if (currentToast->getPhase() == ImGuiToastPhase::Expired)
            {
                ...
                if (rendered_count >= NOTIFY_RENDER_LIMIT)
                {
                    ++i;
                    continue;
                }
