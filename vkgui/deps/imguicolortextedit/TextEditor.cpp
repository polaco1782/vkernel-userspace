...
void TextEditor::Cursors::update() {
	...
	if (hasMultiple()) {
		...
		// merge cursors from the back so erasing later entries keeps earlier
		// cursor indices stable while we coalesce overlapping selections.
		for (size_t index = size() - 1; index > 0;) {
			auto& cursor = at(index);
			auto& previous = at(index - 1);
